...
class Prescription(models.Model):
    ...
    status = models.CharField(max_length=20, choices=[('pending','Pending'),('dispensed','Dispensed')], default='pending')
    def __str__(self):
...
def create_user_profile(sender, instance, created, **kwargs):
    if created:
        if instance.role == 'doctor':
            doctor, _ = Doctor.objects.get_or_create(name=instance.username)
            ...
            pharmacist, _ = Pharmacist.objects.get_or_create(name=instance.username)
            pharmacist.user = instance
            pharmacist.save()
        elif instance.role == 'nurse':
            nurse, _ = Nurse.objects.get_or_create(name=instance.username)
            nurse.user = instance
            nurse.save()
        elif instance.role == 'technician':
            tech, _ = Technician.objects.get_or_create(name=instance.username)
            tech.user = instance
            tech.save()
